...
- `res://items/puck.gd` — add `var thrower: Node = null`, set via a new 4th param on `throw(direction, new_parent, from_global_position, p_thrower)`. `_on_area_entered` becomes `if not is_thrown: return` then `if DamageDispatch.try_deal_damage(area, DAMAGE, STUN_KNOCKBACK, thrower, global_position): queue_free()` (no `exclude` arg passed — **correction, caught during the fix-wave's re-review:** the friendly-fire check alone *does* already cover the thrower's own Hurtbox too, since the thrower is a `PlayerController` and its own hurtbox's `owner_body` is also a `PlayerController` — `DamageDispatch` blocks that regardless of `exclude`. The omission is harmless either way, just for a different reason than originally stated here: it's redundant with the friendly-fire rule, not reliant on the 4px spawn-offset margin. This fix is about giving Puck a `source` for the friendly-fire check, not re-litigating the offset).
